...
export function outRectangleBuffer() {
    const longAlignment = types.long.alignment;
    if (longAlignment !== 4)
        throw Error('long alignment is not 4');
    return Buffer.alloc(4 * longAlignment);
}
export function rectangleOfRectangleBuffer(buffer) {
    const longAlignment = types.long.alignment;
    if (longAlignment !== 4)
        throw Error('long alignment is not 4');
    if (buffer.length !== 4 * longAlignment)
        throw Error('buffer has unexpected size');
    return Rectangle.ofCorners(buffer.readInt32LE(0), buffer.readInt32LE(4), buffer.readInt32LE(8), buffer.readInt32LE(12));
}
